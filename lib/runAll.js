 ...
 * @param {boolean} [options.full] - Check all files in the repo. Overrides `diffRef`.
 * @param {Logger} logger
 * @returns {Promise}
 */
const runAll = async (
  ...
  const ctx = getInitialState({ quiet, diffRef, full })
  ...
  ctx.shouldBackup = hasInitialCommit && stash && !ctx.diffRef && !ctx.full
  if (!ctx.shouldBackup && !ctx.diffRef && !ctx.full) {
